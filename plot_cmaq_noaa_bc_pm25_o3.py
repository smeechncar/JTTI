	...
	for cc in range(n_cycle):
		...
		if cmaq_fname_in_this.is_file():
			...
			if plot_subdomain:
				...
				cmaq_o3   = ds_cmaq.o3_anen_cmaq_bm3[:,:,:]
				cmaq_pm25 = ds_cmaq.pm25_anen_cmaq_bm1[:,:,:]
			...
			suptitle_o3 = 'CMAQ Forecast (NOAA BC approx.)'
			suptitle_pm25 = 'CMAQ Forecast (NOAA BC approx.)'
			if plot_airnow_vals:
			...
			for ll in leads:
				...
				if plot_o3:
					if plot_subdomain:
						fname = out_dir.joinpath('map_calif_cmaq_noaa_bc_o3_'+this_valid_file+'.'+plot_type)
						lg_loc = 'upper left'
						marksize = 81
					else:
						fname = out_dir.joinpath('map_conus_cmaq_noaa_bc_o3_'+this_valid_file+'.'+plot_type)
						lg_loc = 'lower left'
						marksize = 49
...
#					cmap = mpl.cm.rainbow
				...
				if plot_pm25:
					if plot_subdomain:
						fname = out_dir.joinpath('map_calif_cmaq_noaa_bc_pm25_'+this_valid_file+'.'+plot_type)
						lg_loc = 'lower right'
						marksize = 81
					else:
						fname = out_dir.joinpath('map_conus_cmaq_noaa_bc_pm25_'+this_valid_file+'.'+plot_type)
						lg_loc = 'lower left'
						marksize = 49
